...
export function resolveTtsReferenceText(
  ...
  return stripMarkdown(
    text
      ...
      .replace(CG_TOKEN_REGEX, (_, id: string) => cgIdToTitle?.get(id) ?? "Domain")
      .replace(LEGACY_CD_REGEX, (_, id: string) => cgIdToTitle?.get(id) ?? "Domain")
      .replace(LEGACY_CG_REGEX, (_, id: string) => cgIdToTitle?.get(id) ?? "Domain")
