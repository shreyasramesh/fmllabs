...
export function LandingMobileLifeTab({
  ...
  return (
    <div className="flex flex-col pb-8">
      ...
      <Reveal delay={160}>
        <div className="border-t border-[#e8e6dc] px-5 py-4 dark:border-[#3d3d3a]">
          <div className="flex items-baseline justify-between">
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#c96442] dark:text-[#d97757]">Your Life in Weeks</span>
          ...
          <div className="mt-2 flex items-center gap-2">
            <div className="h-1 flex-1 overflow-hidden rounded-full bg-[#e8e6dc] dark:bg-[#3d3d3a]">
              <div className="h-full rounded-full bg-[#c96442] animate-life-progress-fill dark:bg-[#d97757]" style={{ width: `${Math.min(100, lifeCalendar.percentLived)}%`, animationDelay: "300ms" }} />
            </div>
      ...
      <Reveal delay={480}>
        <div className="border-t border-[#e8e6dc] px-5 py-4 dark:border-[#3d3d3a]">
          <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#c96442] dark:text-[#d97757]">Events</p>
          {sorted.length === 0 && <p className="py-3 text-center text-[12px] text-[#87867f]">No events yet</p>}
      ...
      <Reveal delay={560}>
        <div className="border-t border-[#e8e6dc] px-5 py-4 dark:border-[#3d3d3a]">
          {fireTracker ? <FireCard fire={fireTracker} onEdit={onOpenGoals} /> : (
            <>
              <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.3em] text-[#c96442] dark:text-[#d97757]">FIRE Tracker</p>
      ...
      <Reveal delay={640}>
        <div className="px-5 pt-2 pb-2">
          <button type="button" onClick={onOpenGoals} className="w-full rounded-2xl bg-[#faf9f5] py-4 text-center text-[16px] font-bold text-[#141413] shadow-sm transition-all duration-200 hover:shadow-lg active:scale-[0.97] dark:bg-[#30302e] dark:text-[#faf9f5]">
...
// ═══════════════════════════════════════════════════════════════════════════
// Reveal wrapper — animates children into view
// ═══════════════════════════════════════════════════════════════════════════

function Reveal({ children, delay }: { children: React.ReactNode; delay: number }) {
  return (
    <div className="opacity-0 animate-life-card-in" style={{ animationDelay: `${delay}ms` }}>
      {children}
    </div>
  );
}
...
// ═══════════════════════════════════════════════════════════════════════════
// Your Life in Years — staggered dot entrance
// ═══════════════════════════════════════════════════════════════════════════
...
function LifeInYears({ lifeExpectancyYears, yearsLived }: { lifeExpectancyYears: number; yearsLived: number }) {
  ...
  return (
    <div className="border-t border-[#e8e6dc] px-5 py-5 dark:border-[#3d3d3a]">
      <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.3em] text-[#c96442] dark:text-[#d97757]">Your Life in Years</p>
      ...
      <div className="mt-4 flex items-center justify-center gap-5 text-[10px] opacity-0 animate-fade-in" style={{ animationDelay: `${lifeExpectancyYears * 15 + 100}ms` }}>
        <span className="flex items-center gap-1.5"><span className="inline-block h-2 w-2 rounded-full bg-foreground" /><span className="text-[#87867f]">Years lived ({cur})</span></span>
        <span className="flex items-center gap-1.5"><span className="inline-block h-2 w-2 rounded-full bg-foreground/50 ring-1 ring-foreground/30 animate-life-glow-pulse" /><span className="text-[#87867f]">This year</span></span>
        <span className="flex items-center gap-1.5"><span className="inline-block h-2 w-2 rounded-full bg-[#e8e6dc] dark:bg-[#3d3d3a]" /><span className="text-[#87867f]">Ahead ({ahead})</span></span>
      </div>
    </div>
  );
}
...
// ═══════════════════════════════════════════════════════════════════════════
// Year Progress — flip animation on toggle, staggered week dots
// ═══════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div className="border-t border-[#e8e6dc] px-5 py-5 dark:border-[#3d3d3a]">
      <div className="flex items-center justify-between">
        <span className="rounded-full border border-[#e8e6dc] px-2.5 py-0.5 text-[11px] font-bold text-foreground dark:border-[#3d3d3a]">{year}</span>
        <button type="button" onClick={toggle} className="rounded-full p-1 text-[#87867f] transition-all duration-200 hover:text-foreground hover:scale-110 active:scale-90" aria-label="Toggle">
      ...
      <button type="button" onClick={toggle} className="mt-2 flex w-full flex-col items-center py-1" style={{ perspective: "600px" }}>
        ...
        <span key={`lbl-${flipKey}`} className="mt-1 text-[12px] text-[#87867f] opacity-0 animate-fade-in" style={{ animationDelay: "250ms" }}>
          ...
          return (
            <span
              ...
              className={`block h-[7px] w-[7px] rounded-full opacity-0 animate-life-dot-in ${
                isCur
                  ? "animate-life-glow-pulse bg-[#c96442] shadow-[0_0_6px_2px_rgba(201,100,66,0.45)] dark:bg-[#d97757] dark:shadow-[0_0_6px_2px_rgba(217,119,87,0.45)]"
                  : isLived
                    ? "bg-foreground/75"
...
// ═══════════════════════════════════════════════════════════════════════════
// Your Life in Numbers — staggered stat entrance
// ═══════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div className="border-t border-[#e8e6dc] px-5 py-6 dark:border-[#3d3d3a]">
      ...
      <div className="flex flex-col items-center gap-5">
        ...
        <p className="font-mono text-[12px] tabular-nums text-[#87867f] opacity-0 animate-life-number-in" style={{ animationDelay: "400ms" }}>{daysRemaining.toLocaleString()} days remaining</p>
      </div>
    </div>
  );
}
...
// ═══════════════════════════════════════════════════════════════════════════
// Quote bar — slide transition on navigate
// ═══════════════════════════════════════════════════════════════════════════
...
function QuoteBar() {
  ...
  return (
    <div className="flex items-center gap-2 border-t border-[#e8e6dc] px-4 py-3 dark:border-[#3d3d3a]">
      <button type="button" onClick={() => nav(-1)} className="shrink-0 rounded-full p-1 text-[#87867f] transition-all duration-200 hover:text-foreground hover:scale-110 active:scale-90" aria-label="Previous">
      ...
      <button type="button" onClick={() => nav(1)} className="shrink-0 rounded-full p-1 text-[#87867f] transition-all duration-200 hover:text-foreground hover:scale-110 active:scale-90" aria-label="Next">
...
// ═══════════════════════════════════════════════════════════════════════════
// Countdowns — entrance + interactive delete
// ═══════════════════════════════════════════════════════════════════════════

function CountdownRow({ cd, onDelete, delay }: { cd: LifeCountdown; onDelete: () => void; delay: number }) {
  const abs = Math.abs(cd.daysRemaining);
  return (
    <div className="flex items-center gap-2 rounded-xl border border-[#e8e6dc] px-3 py-2.5 opacity-0 animate-life-card-in transition-shadow duration-200 hover:shadow-md dark:border-[#3d3d3a]" style={{ animationDelay: `${delay}ms` }}>
      <div className="min-w-0 flex-1">
        <p className="truncate text-[13px] font-semibold text-foreground">{cd.label}</p>
        <p className="text-[10px] text-[#87867f]">{cd.isPast ? `${abs}d ago` : `${abs}d left`} &middot; {cd.targetDate}</p>
      </div>
...
function AddCountdown({ onAdd, disabled }: { onAdd: (l: string, d: string) => void; disabled: boolean }) {
  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setOpen(true)} disabled={disabled} className="w-full rounded-xl border border-dashed border-[#d1cfc5] py-2.5 text-[12px] font-medium text-[#87867f] transition-all duration-200 hover:border-[#c96442] hover:text-[#c96442] hover:shadow-sm active:scale-[0.98] disabled:opacity-40 dark:border-[#4d4c48] dark:hover:border-[#d97757] dark:hover:text-[#d97757]">
        + Add event
      </button>
    );
  }
  ...
  return (
    <div className="space-y-2 rounded-xl border border-[#e8e6dc] p-2.5 animate-life-card-in dark:border-[#3d3d3a]">
      <input type="text" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Event name" maxLength={100} autoFocus className="w-full rounded-lg border border-[#e8e6dc] bg-background px-2.5 py-1.5 text-[13px] outline-none transition-shadow duration-200 focus:border-[#c96442] focus:ring-2 focus:ring-[#c96442]/20 dark:border-[#3d3d3a]" />
      <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full rounded-lg border border-[#e8e6dc] bg-background px-2.5 py-1.5 text-[13px] outline-none transition-shadow duration-200 focus:border-[#c96442] focus:ring-2 focus:ring-[#c96442]/20 dark:border-[#3d3d3a]" />
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => setOpen(false)} className="flex-1 rounded-lg border border-[#e8e6dc] py-1.5 text-[12px] text-[#87867f] transition-all duration-200 hover:bg-[#f0eee6] active:scale-[0.97] dark:border-[#3d3d3a] dark:hover:bg-[#3d3d3a]">Cancel</button>
      </div>
    </div>
  );
}
...
function FireCard({ fire, onEdit }: { fire: FireTrackerData; onEdit: () => void }) {
  ...
  return (
    <>
      <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[#c96442] dark:text-[#d97757]">FIRE Tracker</p>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {fire.projectedYearsToTarget != null && <p className="text-[10px] text-[#87867f]">~{fire.projectedYearsToTarget.toFixed(1)} yr to target</p>}
        </div>
      </div>
      <div className="mt-3 grid grid-cols-4 gap-1.5 text-center">
        {([["Saved", fmt$(fire.savingsCurrent)], ["Target", fmt$(fire.targetAmount)], ["Monthly", fmt$(fire.monthlyContribution)], ["Return", `${fire.annualReturnPct}%`]] as const).map(([l, v]) => (
          <div key={l} className="rounded-lg bg-[#f0eee6]/60 px-1 py-1.5 transition-shadow duration-200 hover:shadow-sm dark:bg-[#3d3d3a]/40">
            <p className="text-[11px] font-bold tabular-nums text-foreground">{v}</p>
            <p className="text-[9px] text-[#87867f]">{l}</p>
          </div>
        ))}
      </div>
      <button type="button" onClick={onEdit} className="mt-3 w-full rounded-lg border border-[#e8e6dc] py-1.5 text-[11px] font-medium text-[#87867f] transition-all duration-200 hover:bg-[#f0eee6] hover:shadow-sm active:scale-[0.98] dark:border-[#3d3d3a] dark:hover:bg-[#3d3d3a]">Edit FIRE settings</button>
    </>
  );
}

// ═══════════════════════════════════════════════════════════════════════════
// Birthday setup — cascading entrance
// ═══════════════════════════════════════════════════════════════════════════
...
function BirthdaySetup({ onSave }: { onSave: (b: string) => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 px-5 py-10 text-center animate-life-card-in">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="h-12 w-12 text-[#c96442] animate-life-glow-pulse dark:text-[#d97757]">
      ...
      <div className="opacity-0 animate-life-number-in" style={{ animationDelay: "150ms" }}>
        ...
        <p className="mt-1 text-[12px] leading-relaxed text-[#87867f]">Enter your birthday to unlock the life calendar.<br />Your data stays completely private.</p>
      </div>
      <input type="date" value={val} onChange={(e) => setVal(e.target.value)} max={new Date().toISOString().slice(0, 10)} className="w-full max-w-[200px] rounded-xl border border-[#e8e6dc] bg-background px-3 py-2.5 text-center text-[14px] opacity-0 animate-life-number-in outline-none transition-shadow duration-200 focus:border-[#c96442] focus:ring-2 focus:ring-[#c96442]/20 dark:border-[#3d3d3a]" style={{ animationDelay: "300ms" }} />
